...
namespace LösenRenskriv
{
    public class Program
    {
        ...
        {
            //spara och jsonizea i set, delete, secret
            //Get("client.json", "server.json", "");

            //fixa efter result != null, inte deserialisera igen ???
            ...
            if (args.Length == 0)
                ...
                switch (args[0].ToLower())
                {
                    ...
                    try
                    {
                        ...
                        {
                            string prop = args[3];
                            GetPass(clientPath, serverPath, prop);
                        }

                        else if (args[3] == null)
                        {
                            string noProp = "noProp";
                            GetProps(clientPath, serverPath);
                        }
                    }
                    catch
                    {
            ...
            static void Delete(string clientP, string serverP, string prop)
            {
                ...
                try
                {
                    ...
                    byte[] byteVault = Convert.FromBase64String(serverJsonText["EncryptedVault"]);
                    byte[] byteIV = Convert.FromBase64String(serverJsonText["IV"]);

                    //hämta client-filen, konvertera inför dekrypteringen
                    string readJsonClient = File.ReadAllText(clientP);
                    Dictionary<string, string> clientJsonText = JsonSerializer.Deserialize<Dictionary<string, string>>(readJsonClient);
                    string secretKey = clientJsonText["SecretKey"];
                    byte[] secretKeyByte = Convert.FromBase64String(secretKey);

                    //gör vaultkey till dekrypteringen
                    int iterera = 1000;
                    Rfc2898DeriveBytes newVaultKey = new Rfc2898DeriveBytes(masterPassword, secretKeyByte, iterera);
                    byte[] byteNewVaultKey = newVaultKey.GetBytes(16);

                    //dekryptera valvet till string
                    string decryptedVault = DecryptStringFromBytes_Aes(byteVault, byteNewVaultKey, byteIV);

                    //deserialisera till dictionary
                    Dictionary<string, string> decryptedVaultDic = JsonSerializer.Deserialize<Dictionary<string, string>>(decryptedVault);

                    if (decryptedVaultDic.ContainsKey(prop))
                    {
                        decryptedVaultDic.Remove(prop);
                    ...
                    //vault till string för att kryptera
                    string vaultText = JsonSerializer.Serialize(decryptedVaultDic);
                    byte[] encryptedVault = EncryptStringToBytes_Aes(vaultText, byteNewVaultKey, byteIV);

                    //konvertera för att kunna lägga in i dictionary 
                    string encryptedVaultText = Convert.ToBase64String(encryptedVault);
                    ...
                    string jsonServer = JsonSerializer.Serialize(serverDic);
                    File.WriteAllText(serverP, jsonServer);

                }
                catch
                {
                    Console.WriteLine("metod DEL Wrong input.");
                ...
                //        string jsonServer = JsonSerializer.Serialize(decryptedVault);
            ...
            static void Set(string clientP, string serverP, string prop, bool generate)
            {
                ...
                try
                {
                    //hämta server-filen, konvertera inför dekrypteringen
                    string readJsonServer = File.ReadAllText(serverP);
                    Dictionary<string, string> serverJsonText = JsonSerializer.Deserialize<Dictionary<string, string>>(readJsonServer);
                    byte[] byteVault = Convert.FromBase64String(serverJsonText["EncryptedVault"]);
                    byte[] byteIV = Convert.FromBase64String(serverJsonText["IV"]);
                    ...
                    if (generate == true)
                    {
                        ...
                        decryptedVaultDic[prop] = generatedPassword;
                    ...
                    else if (generate == false)
                    {
                        ...
                        {
                            decryptedVaultDic[prop] = newPassword;
                            Console.WriteLine($"Your new password for {prop} is: {newPassword}"); //ska detta skrivas ut??
                        }
                ...
                {
                    Console.WriteLine("metod SET Wrong input.");
                }
                ...
                //        Console.WriteLine($"Your generated password for {prop} is: {generatedPassword}");
                //        decryptedVault[prop] = generatedPassword;
                //    }
                //    else if (generate == false)
                //    {
                //        Console.WriteLine($"Enter your new password for {prop}:");
            ...
            static void GetPass(string clientP, string serverP, string prop)
            {
                ...
                try
                {
                    ...
                    //se om dic innehåller key prop
                    if (decryptedVaultDic.ContainsKey(prop))
                    {
                        Console.WriteLine($"This is the password for {prop}:");
                        Console.WriteLine(decryptedVaultDic["prop"]);
                    }
                    else Console.WriteLine("Vault contains no such property.");
                }
                catch
                {
                    Console.WriteLine("metod GET pass Wrong input.");
                }
                ...
                //{
                //    //hämta server-filen, konvertera inför dekrypteringen
                //    Dictionary<string, string> serverJson = JsonSerializer.Deserialize<Dictionary<string, string>>(serverP);
                //    byte[] byteVault = Convert.FromBase64String(serverJson["EncryptedVault"]);
                //    byte[] byteIV = Convert.FromBase64String(serverJson["IV"]);
            ...
            static void GetProps(string clientP, string serverP)
            {
                Console.WriteLine("Enter your password:");
                string masterPassword = Console.ReadLine();
                ...
                try
                {
                    ...
                    foreach (string key in decryptedVaultDic.Keys)
                    {
                ...
                {
                    Console.WriteLine("metod GETprop Wrong input.");
                }
            ...
            static void Create(string clientP, string serverP)
            {
                ...
                //hsecret key + masterpassword skapar vault key, hämta encrpted vault & IV & deserializea + konverterra, decrypta m dessa tre
            ...
            static void Init(string clientP, string serverP)
            {
                ...
                using (Aes AES = Aes.Create())
                {
                    ...
                    //gör om till string för att kunna lagra i en dictionary
                    string IVtext = Convert.ToBase64String(IV);
                    string encryptedVaultText = Convert.ToBase64String(encryptedVault);
            ...
            static string CheckMasterPassword(string input, string clientP, string serverP)
            {
                ...
                //gör vaultkey mha masterpass och secret key
                int iterera = 1000;
